...
## 📚 Catálogo Canônico de Skills Portáteis (13 Runbooks)

O ecossistema disponibiliza 13 habilidades cognitivas universais organizadas por domínio de especialidade:
...
| **Cloud no FreeBSD**          | [`freebsd-cloud`](./freebsd-cloud/SKILL.md)                         | Arquitetura e operação em nuvem FreeBSD moderna (Jails, Bastille, Podman nativo com runj, imagens OCI no Docker Hub, bhyve e ZFS). | Desenvolvimento de serviços para FreeBSD, conteinerização OCI com Podman, Jails VNET e ZFS storage.     |
| **Cloud no Linux**            | [`linux-cloud`](./linux-cloud/SKILL.md)                             | Arquitetura e microsserviços em nuvem Linux moderna (Podman rootless/Quadlet, Incus/LXC, systemd-nspawn, eBPF e Cloud-Init).       | Implantação de microsserviços, containers rootless, automação de servidores e infraestrutura Linux.     |
| **Cloud no illumos**          | [`illumos-cloud`](./illumos-cloud/SKILL.md)                         | Arquitetura de infraestrutura corporativa illumos (SmartOS, OmniOS, Zones nativas e lx-brand, Crossbow, ZFS, SMF e DTrace).        | Projetos de alta densidade, virtualização de rede com Crossbow, auditoria com DTrace e SMF.             |
